...
namespace PeterDBTesting {
    ...
    TEST_F(RBFM_Test, insert_and_read_a_record_with_null) {
        ...
        rbfm.printRecord(recordDescriptor, inBuffer, stream);
        ASSERT_NO_FATAL_FAILURE(
                checkPrintRecord("EmpName: Anteater, Age: NULL, Height: NULL, Salary: NULL", stream.str()));
    ...
    TEST_F(RBFM_Test, insert_and_read_1000_records) {
        // Functions tested
        // 1. Create Record-Based File
        // 2. Open Record-Based File
        ...
        for (int i = 0; i < numRecords; i++) {
            ...
            ASSERT_EQ(rbfm.insertRecord(fileHandle, recordDescriptor, inBuffer, rid), success)
                                        << "Inserting a inBuffer should succeed.";
//            if(i == 0){
//                std::cout<<"index 0 inbuffer: ";
//                for(int j = 0; j < size; j++){
//                    std::cout<<*((char *)inBuffer + j);
//                }
//                std::cout<<std::endl;
//            }
        ...
        std::cout<<"file size:"<<totalfilesize<<std::endl;
...
//            }
            ...
            int size = 0;
            prepareLargeRecord(recordDescriptor.size(), nullsIndicator, i, inBuffer, &size);
            ASSERT_EQ(memcmp(outBuffer, inBuffer, sizes[i]), 0) << "the read data should match the inserted data, index = " << i
                                                                <<"rid pagenum: "<<rids[i].pageNum << "rid slotnum:"<<rids[i].slotNum
