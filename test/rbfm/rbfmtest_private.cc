...
namespace PeterDBTesting {
    ...
    TEST_F(RBFM_Private_Test, measure_time_complexity) {
        ...
        std::vector<PeterDB::RID> ridList;
        size_t recordSize = 0;
        inBuffer = malloc(100);
        ...
        auto expectedWrite = [&](std::vector<unsigned>& metrics) {
            // skip check write complexity
            return;
        };

        // Check if insertRecord executes in O(n) page read operations
        checkTimeComplexity(evaluateFunc, expectedWrite, metricFunc, numRecords / roundSize);

        auto evaluateFunc2 = [&](int round) {
            for (int i = 0; i < roundSize; i++) {
                PeterDB::RID rid = ridList.at(round * roundSize + i);
                ASSERT_EQ(rbfm.readRecord(fileHandle, recordDescriptor, rid, outBuffer), success)
        ...
        auto expectedRead = [&](std::vector<unsigned>& metrics) {
            if (metrics.size() < 2) {
                return;
            }
            double upperBound = metrics.at(0) * 1.2;
            unsigned current = metrics.at(metrics.size() - 1);
            if (current > upperBound) {
                ...
                std::string metricsStr = ss.str();
                ASSERT_TRUE(false) << "expected O(1) time complexity, actual metrics delta: " << metricsStr;
            }
        };

        // Check if readRecord executes in O(1) page read operations
        checkTimeComplexity(evaluateFunc2, expectedRead, metricFunc2, numRecords / roundSize);
